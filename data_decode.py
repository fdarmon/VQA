...
def show_results(test_pred):
    ...
    with open('data_prepro.json') as f:
        vocab=json.load(f)

    vocab_ans=vocab["ix_to_ans"]
    ...
    for annot in ans_test['annotations']:
        test_true['ans'].append(annot['multiple_choice_answer'])
        test_true['cat'].append(annot['answer_type'])
